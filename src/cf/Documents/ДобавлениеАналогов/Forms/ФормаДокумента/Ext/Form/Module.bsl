
&НаКлиенте
Процедура ДобавитьПрепаратыБезАналогов(Команда)
	ДобавитьПрепаратыБезАналоговНаСервере();
КонецПроцедуры

&НаСервере
Процедура ДобавитьПрепаратыБезАналоговНаСервере()
	
    Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	АналогиПрепаратов.Препарат КАК Препарат
	               |ПОМЕСТИТЬ ВТ_Аналоги
	               |ИЗ
	               |	РегистрСведений.АналогиПрепаратов КАК АналогиПрепаратов
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	Препараты.Ссылка КАК Препарат
	               |ПОМЕСТИТЬ ВТ_ВсеПрапараты
	               |ИЗ
	               |	Справочник.Препараты КАК Препараты
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_ВсеПрапараты.Препарат КАК Препарат
	               |ИЗ
	               |	ВТ_Аналоги КАК ВТ_Аналоги
	               |		ПРАВОЕ СОЕДИНЕНИЕ ВТ_ВсеПрапараты КАК ВТ_ВсеПрапараты
	               |		ПО ВТ_Аналоги.Препарат = ВТ_ВсеПрапараты.Препарат
	               |ГДЕ
	               |	ВТ_Аналоги.Препарат ЕСТЬ NULL";
	Выборка = Запрос.Выполнить().Выбрать(); 
	//Выборка.Следующий();
	Пока Выборка.Следующий() Цикл 
		НоваяСтрока = Объект.СписокАналогов.Добавить(); 
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
	КонецЦикла;
	
КонецПроцедуры


&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	УстановитьУсловноеОформление();
	
КонецПроцедуры   

#Область Команды


&НаКлиенте
Процедура ОчиститьТаблицу(Команда)
	
	Объект.СписокАналогов.Очистить();
	
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьКорректностьДанных() 
	
	Сообщение = Новый СообщениеПользователю;    
	Если Объект.СписокАналогов.Количество() = 0 Тогда
		//ПоказатьПредупреждение(,СообщенияПользователю().НезаполненаТЧ);
		Сообщение.Текст = СообщенияПользователю().НезаполненаТЧ;
		Сообщение.Сообщить();
	Иначе 
		СгруппироватьСтроки();
		УдалитьЗеркальныеСтроки();
		Для каждого Элемент из Объект.СписокАналогов Цикл
			Элемент.ЕстьОшибка = 2;
		КонецЦикла;  
		
		КоличествоПустыхСтрок = КоличествоПустыхСтрок().Количество();
		Если  КоличествоПустыхСтрок  > 0 Тогда
			ПоказатьПредупреждение(Новый ОписаниеОповещения("ВывестиОшибкиПустыхСтрок", ЭтотОбъект, КоличествоПустыхСтрок()), 
			СообщенияПользователю().ЕстьНезаполненныеСтроки);  
		Иначе	
			ПроверитьКорректностьДанныхНаСервере();
		КонецЕсли;  
	КонецЕсли;
	
КонецПроцедуры


#КонецОбласти


&НаСервере
Функция СообщитьРезультат()
	
	МассивСтрок = Объект.СписокАналогов.НайтиСтроки(Новый Структура("ЕстьОшибка",2));
	Если МассивСтрок.Количество() = Объект.СписокАналогов.Количество() Тогда
		Возврат СообщенияПользователю().ПроверкаПройдена;
	КонецЕсли;
	
	МассивСтрок = Объект.СписокАналогов.НайтиСтроки(Новый Структура("ЕстьОшибка",0));
	Если МассивСтрок.Количество() <> 0 Тогда
		Возврат СообщенияПользователю().ОтсутствуетПроверкаКорректности;
	Иначе
		Возврат  СообщенияПользователю().ЕстьОшибки;
	КонецЕсли;
	
КонецФункции	

&НаСервере
Процедура СгруппироватьСтроки() 
	
	Запрос = Новый Запрос; 
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТЗ.Препарат КАК Препарат,
	               |	ТЗ.Аналог КАК Аналог,
	               |	ТЗ.Приоритет КАК Приоритет
	               |ПОМЕСТИТЬ ВТ_тч
	               |ИЗ
	               |	&ТЗ КАК ТЗ
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_тч.Препарат КАК Препарат,
	               |	ВТ_тч.Аналог КАК Аналог,
	               |	ВТ_тч.Приоритет КАК Приоритет
	               |ИЗ
	               |	ВТ_тч КАК ВТ_тч
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_тч.Препарат,
	               |	ВТ_тч.Аналог,
	               |	ВТ_тч.Приоритет";
	Запрос.УстановитьПараметр("ТЗ", Объект.СписокАналогов.Выгрузить()); 
	Объект.СписокАналогов.Очистить();
	Объект.СписокАналогов.Загрузить(Запрос.Выполнить().Выгрузить());
	
КонецПроцедуры	

&НаСервере
Процедура УдалитьЗеркальныеСтроки() 
	
	ТЧАналоги = Объект.СписокАналогов;
	Индекс = ТЧАналоги.Количество() - 1; 
	
	Пока Индекс > 0 Цикл
		ТекАналог = ТЧАналоги[Индекс].Аналог;
		ТекПрепарат = ТЧАналоги[Индекс].Препарат;
		МассивСтрок = ТЧАналоги.НайтиСтроки(Новый Структура("Препарат, Аналог", ТекАналог, ТекПрепарат)); 
		Для каждого Строка из МассивСтрок Цикл
			ТЧАналоги.Удалить(Строка);
		КонецЦикла;	
		Индекс = Индекс - 1;
	КонецЦикла;	
	
КонецПроцедуры

&НаСервере
Функция КоличествоПустыхСтрок()   
	
	МассивПустыхСтрок = Новый Массив;
	Для каждого Строка из Объект.СписокАналогов Цикл
		Если НЕ ЗначениеЗаполнено(Строка.Препарат) ИЛИ НЕ ЗначениеЗаполнено(Строка.Аналог) Тогда 
			МассивПустыхСтрок.Добавить(Строка.НомерСтроки);
		КонецЕсли;
	КонецЦикла; 
	
	Возврат МассивПустыхСтрок;
	
КонецФункции

&НаКлиенте
Процедура ВывестиОшибкиПустыхСтрок(ДополнительныеПараметры) Экспорт
	
	Для каждого ЭлМассива из ДополнительныеПараметры Цикл
		Сообщение = Новый СообщениеПользователю;    
		Сообщение.Текст = СообщенияПользователю().НезаполненаСтрока + ЭлМассива;
		Сообщение.Сообщить(); 
		Объект.СписокАналогов[ЭлМассива - 1].ЕстьОшибка = 1;
	КонецЦикла;	
	
КонецПроцедуры	

&НаСервере
Процедура ПроверитьКорректностьДанныхНаСервере() 
	
	СтруктураОшибок = Новый Структура;
	ВыгрузкаПрепаратовТЧ = Объект.СписокАналогов.Выгрузить(); 
	
	Запрос = Новый Запрос;
	Запрос.Текст =  "ВЫБРАТЬ
	|	АналогиПрепаратов.Препарат КАК Препарат,
	|	АналогиПрепаратов.Аналог КАК Аналог
	|ПОМЕСТИТЬ ВТ_АналогиРег
	|ИЗ
	|	РегистрСведений.АналогиПрепаратов КАК АналогиПрепаратов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЗ.Препарат КАК Препарат,
	|	ТЗ.Аналог КАК Аналог
	|ПОМЕСТИТЬ ВТ_ТЗ
	|ИЗ
	|	&ТЗ КАК ТЗ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_АналогиРег.Препарат КАК Препарат,
	|	ВТ_АналогиРег.Аналог КАК Аналог
	|ИЗ
	|	ВТ_АналогиРег КАК ВТ_АналогиРег
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТЗ КАК ВТ_ТЗ
	|		ПО ВТ_АналогиРег.Препарат = ВТ_ТЗ.Препарат
	|			И ВТ_АналогиРег.Аналог = ВТ_ТЗ.Аналог";  
	Запрос.УстановитьПараметр("ТЗ",ВыгрузкаПрепаратовТЧ);
	ВыборкаПрепаратов = Запрос.Выполнить().Выбрать();
	
	Если ВыборкаПрепаратов.Следующий() Тогда
		
		Для каждого Строка из Объект.СписокАналогов Цикл
			Если Строка.Препарат = Строка.Аналог Тогда
				Сообщение = Новый СообщениеПользователю;    
				Сообщение.Текст = СообщенияПользователю().ИдентичностьПозицийВСтроке + Строка.НомерСтроки;
				Сообщение.Сообщить(); 
				Строка.ЕстьОшибка = 1;
			КонецЕсли;
			ВыборкаПрепаратов.Сбросить();
			Если ВыборкаПрепаратов.НайтиСледующий(Новый Структура("Препарат, Аналог",Строка.Препарат, Строка.Аналог)) Тогда 
				Сообщение = Новый СообщениеПользователю;    
				Сообщение.Текст = СообщенияПользователю().ОригинальностьПрепаратов + Строка.НомерСтроки;
				Сообщение.Сообщить();
				Строка.ЕстьОшибка = 1;
			КонецЕсли;
		КонецЦикла;	
		
	Иначе   
		
		Для каждого Строка из Объект.СписокАналогов Цикл
			Если Строка.Препарат = Строка.Аналог Тогда 
				Сообщение = Новый СообщениеПользователю;    
				Сообщение.Текст = СообщенияПользователю().ИдентичностьПозицийВСтроке + Строка.НомерСтроки;
				Сообщение.Сообщить(); 
				Строка.ЕстьОшибка = 1;
			КонецЕсли;
		КонецЦикла; 
		
	КонецЕсли; 	
	
КонецПроцедуры

&НаСервере
Процедура УстановитьУсловноеОформление()
	
	УсловноеОформлениеДляСпискаАналогов(); 
	УсловноеОформлениеДляЗеркальногоСпискаАналогов();
		
КонецПроцедуры	

&НаСервере
Процедура УсловноеОформлениеДляСпискаАналогов() 
   ОбщегоНазначенияКлиентСервер.ПолучитьОбъект(УсловноеОформление.Элементы, ЭтотОбъект.ИмяФормы, Элементы.СписокАналогов.ПодчиненныеЭлементы);
КонецПроцедуры	

&НаСервере
Процедура УсловноеОформлениеДляЗеркальногоСпискаАналогов()
	
	 Элементы.СписокАналоговЗеркальныйАналог.ТолькоПросмотр = Истина;
	 Элементы.СписокАналоговЗеркальныйПрепарат.ТолькоПросмотр = Истина;
	 Элементы.СписокАналоговЗеркальныйНомерСтроки.ТолькоПросмотр = Истина;
	
КонецПроцедуры	

&НаСервере
Функция СообщенияПользователю()
	
	СтруктураСообщений = Новый Структура;
	СтруктураСообщений.Вставить("ИдентичностьПозицийВСтроке",
					НСтр("ru = 'Значение препарата совпадает с аналогом в строке № ' ; en = 'The value of the drug coincides with the analog in line №' "));
	СтруктураСообщений.Вставить("АналогиДобавлены", НСтр("ru = ' Аналоги добавлены!'; en = 'Analogs added!'"));  
	СтруктураСообщений.Вставить("АналогиНеДобавлены",НСтр("ru = 'Аналоги не были добавлены!'; en = 'Analogues have not been added!'"));
	СтруктураСообщений.Вставить("ОригинальностьПрепаратов",
					НСтр("ru = 'Препарат уже существует в базе. Ошибка в строке №'; en = 'The drug already exists in the database. Error in line №'"));  
	СтруктураСообщений.Вставить("НезаполненаТЧ", НСтр("ru = ' Список не заполнен!'; en = 'The list is not filled!'"));							
	СтруктураСообщений.Вставить("НезаполненаСтрока", НСтр("ru = ' Не заполнена строка №'; en = 'The line is not filled in. Error in line №'"));	
	СтруктураСообщений.Вставить("ЕстьНезаполненныеСтроки",
					НСтр("ru = ' В списке присутствуют незаполненные строки'; en = 'There are blank lines in the list'"));
	СтруктураСообщений.Вставить("ЕстьОшибки", НСтр("ru = ' Есть ошибки. Записи не добавлены.'; en = 'There are mistakes. No entries have been added.'"));							
	СтруктураСообщений.Вставить("ОтсутствуетПроверкаКорректности", 
					НСтр("ru = ' Необходимо установить проверку, нажав на соответствующую кнопку.'; en = 'You need to install the check by clicking on the appropriate button.'"));
	
	СтруктураСообщений.Вставить("ПроверкаПройдена", НСтр("ru = 'Проверка пройдена!'; en = 'The check is passed!'"));				
	
	Возврат СтруктураСообщений;
	
КонецФункции	

&НаКлиенте
Процедура СписокАналоговПриИзменении(Элемент)
	
    ЗаполнитьЗеркальнуюТЧ();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЗеркальнуюТЧ()  
	
	Объект.СписокАналоговЗеркальный.Очистить();
	Для каждого Строка из Объект.СписокАналогов Цикл  
		Строка.ЕстьОшибка = 0;  
		Если ЗначениеЗаполнено(Строка.Препарат) И ЗначениеЗаполнено(Строка.Аналог) Тогда
			НоваяСтрока = Объект.СписокАналоговЗеркальный.Добавить(); 
			НоваяСтрока.Препарат = Строка.Аналог;
			НоваяСтрока.Аналог = Строка.Препарат;
		КонецЕсли;
	КонецЦикла;  
	
КонецПроцедуры	

&НаКлиенте
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи) 
	
	ПроверитьКорректностьДанных();
	Если СообщитьРезультат() <> СообщенияПользователю().ПроверкаПройдена Тогда
		Сообщение = Новый СообщениеПользователю;    
		Сообщение.Текст = СообщитьРезультат();
		Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			Отказ = Истина;
		КонецЕсли;	
	КонецЕсли;  

КонецПроцедуры

&НаСервере
Функция НайтиСоздатьЭлементСправочникаПоНаименованию(ИмяСпрПрепараты, НаименованиеСпрПрепараты, ИмяСпрКомпоненты, НаименованиеСпрКомпоненты)
	
	ОбъектСпрПрепарат = Справочники[ИмяСпрПрепараты]; 
	//++
	ОбъектСпрКомпонент = Справочники[ИмяСпрКомпоненты];
	РезультатПоиска_Компонент = ОбъектСпрПрепарат.НайтиПоНаименованию(НаименованиеСпрКомпоненты);
	//++
	РезультатПоиска_Препарат = ОбъектСпрПрепарат.НайтиПоНаименованию(НаименованиеСпрПрепараты);

	Если ЗначениеЗаполнено(РезультатПоиска_Препарат) Тогда
		Возврат РезультатПоиска_Препарат;
	Иначе
		НовыйЭлемент = ОбъектСпрПрепарат.СоздатьЭлемент();
		НовыйЭлемент.Наименование = НаименованиеСпрПрепараты;
		//++
		НовыйЭлемент.ДействующееВещество = ?(ЗначениеЗаполнено(РезультатПоиска_Компонент), РезультатПоиска_Компонент, НовыйЭлСпрКомпонент(ОбъектСпрКомпонент, НаименованиеСпрКомпоненты)); 
		//++
		НовыйЭлемент.Записать();
		Возврат НовыйЭлемент.Ссылка;
	КонецЕсли;
	
КонецФункции 

&НаСервере
Функция НовыйЭлСпрКомпонент(ОбъектСпрКомпонент, НаименованиеСпрКомпоненты)
	
	НовыйКомпонент = ОбъектСпрКомпонент.СоздатьЭлемент();
	НовыйКомпонент.Наименование = НаименованиеСпрКомпоненты;
	НовыйКомпонент.Записать();
	
	Возврат НовыйКомпонент.Ссылка;  
	
КонецФункции	

&НаСервере
Процедура ПрочитатьВыбранныйФайл(АдресВХранилище, РасширениеФайла)

	НашиДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресВХранилище);
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла(РасширениеФайла);
	НашиДвоичныеДанные.Записать(ИмяВременногоФайла);
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.Прочитать(ИмяВременногоФайла, СпособЧтенияЗначенийТабличногоДокумента.Значение);
	УдалитьФайлы(ИмяВременногоФайла);
	ОписаниеТипа_Число = Новый ОписаниеТипов("Число");
	
	// Чтение данных
	// Просмотр колонок первой строки, по именам найдем колонки из которых будем читать
	НомерКолонки_Препарат = 0;
	НомерКолонки_ДействующееВещ = 0;
	НомерКолонки_Аналог = 0;
	НомерКолонки_Приоритет = 0; 
	
	Для НомерКолонки = 1 По ТабДок.ШиринаТаблицы Цикл
		
		ИмяКолонки = ТабДок.Область(1, НомерКолонки, 1, НомерКолонки).Текст;
		Если ИмяКолонки = "Препарат" Тогда
			НомерКолонки_Препарат = НомерКолонки;
		ИначеЕсли ИмяКолонки = "Аналог" Тогда
			НомерКолонки_Аналог = НомерКолонки;
		ИначеЕсли ИмяКолонки = "Приоритет" Тогда
			НомерКолонки_Приоритет = НомерКолонки; 
			//++
		ИначеЕсли ИмяКолонки = "Действующее вещество препарата" Тогда
			НомерКолонки_ДействующееВещПР = НомерКолонки; 
		ИначеЕсли ИмяКолонки = "Действующее вещество аналога" Тогда
			НомерКолонки_ДействующееВещАН = НомерКолонки; 
			//++
		КонецЕсли; 
		
	КонецЦикла;
	
	// Проверка, все ли колонки нашли
	Если (НомерКолонки_Препарат = 0) или (НомерКолонки_Аналог = 0) или 
		(НомерКолонки_Приоритет = 0) или (НомерКолонки_ДействующееВещПР = 0) или (НомерКолонки_ДействующееВещАН = 0) Тогда  //++ to do
		
		СообщениеПользователю = Новый СообщениеПользователю;
		СообщениеПользователю.Текст = "Проверьте названия колонок в файле в первой строке, необходимо наличие колонок с наименованиями:" + 
					"Препарат, " + "Аналог, " + "Приоритет, " + "Действующее вещество препарата, " + "Действующее вещество аналога";  //++ 
		СообщениеПользователю.Сообщить();
		
		Возврат;
	КонецЕсли;
	
	// Читаем строки
	НомерКонечнойСтроки = ТабДок.ВысотаТаблицы;
	Для НомерСтрокиВТаблице = 2 По НомерКонечнойСтроки Цикл
		
		Поле_Препарат = ТабДок.Область(НомерСтрокиВТаблице, НомерКолонки_Препарат, НомерСтрокиВТаблице, НомерКолонки_Препарат).Текст;
		Поле_Аналог = ТабДок.Область(НомерСтрокиВТаблице, НомерКолонки_Аналог, НомерСтрокиВТаблице, НомерКолонки_Аналог).Текст;
		Поле_Приоритет = ТабДок.Область(НомерСтрокиВТаблице, НомерКолонки_Приоритет, НомерСтрокиВТаблице, НомерКолонки_Приоритет).Текст; 
		
		//++
		Поле_ДействующееВещПР = ТабДок.Область(НомерСтрокиВТаблице, НомерКолонки_ДействующееВещПР, НомерСтрокиВТаблице, НомерКолонки_ДействующееВещПР).Текст;
		Поле_ДействующееВещАН = ТабДок.Область(НомерСтрокиВТаблице, НомерКолонки_ДействующееВещАН, НомерСтрокиВТаблице, НомерКолонки_ДействующееВещАН).Текст;
		//++ 
		
		НашПрепарат = НайтиСоздатьЭлементСправочникаПоНаименованию("Препараты", Поле_Препарат, "Компоненты", Поле_ДействующееВещПР);  //++ 
		
		НашАналог = НайтиСоздатьЭлементСправочникаПоНаименованию("Препараты", Поле_Аналог, "Компоненты", Поле_ДействующееВещАН);  //++ 
		НашПриоритетЧисло = Мин(Макс(ОписаниеТипа_Число.ПривестиЗначение(Поле_Приоритет), 1), 5);
		НашПриоритет = Перечисления.Приоритеты.Получить(НашПриоритетЧисло); 
		
		// Справочники нашли, добавим в документ
		НоваяСтрока = Объект.СписокАналогов.Добавить();
		НоваяСтрока.Препарат = НашПрепарат;
		НоваяСтрока.Аналог = НашАналог;
		НоваяСтрока.Приоритет = НашПриоритет;
	
	КонецЦикла; 
	
     	ЗаполнитьЗеркальнуюТЧ();

КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	Если ВыбранныеФайлы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	НашиДвоичныеДанные = Новый ДвоичныеДанные(ВыбранныеФайлы[0]);
	АдресВХранилище = ПоместитьВоВременноеХранилище(НашиДвоичныеДанные, ЭтаФорма.УникальныйИдентификатор);
	//ТабличныйДокумент = Элементы.ТабДокумент;
	ФайлНаДиске = Новый Файл(ВыбранныеФайлы[0]);

	Если нРег(ФайлНаДиске.Расширение) = ".mxl" Тогда
		//мПрочитатьТабличныйДокументИзMXL(ДиалогВыбораФайла.ПолноеИмяФайла);

	ИначеЕсли нРег(ФайлНаДиске.Расширение) = ".xls" Тогда
		ПрочитатьВыбранныйФайл(АдресВХранилище, ФайлНаДиске.Расширение);

	ИначеЕсли нРег(ФайлНаДиске.Расширение) = ".xlsx" Тогда
		//мПрочитатьТабличныйДокументИзExcel(ДиалогВыбораФайла.ПолноеИмяФайла);
	КонецЕсли;
	
	
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьИзExcel(Команда)
	
	ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогВыбораФайла.Заголовок = "Прочитать табличный документ из файла";
	ДиалогВыбораФайла.Фильтр    = "Лист Excel (*.xls)|*.xls|";
	// ДиалогВыбораФайла.Фильтр    = "Табличный документ (*.mxl)|*.mxl|Лист Excel (*.xls)|*.xls|Лист Excel 2007 (*.xlsx)|*.xlsx|";
	ДиалогВыбораФайла.МножественныйВыбор = Ложь;
	
	ОписаниеОповещенияДляОткрытия = Новый ОписаниеОповещения("ПослеВыбораФайла", ЭтаФорма);
	ДиалогВыбораФайла.Показать(ОписаниеОповещенияДляОткрытия);  
    
КонецПроцедуры



